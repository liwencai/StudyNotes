...
<html>
	...
	<body>
		<div id="app">
			<!-- 方法 -->
			 <h2>{{getMethodsFullName()}}</h2>
			 <!-- 计算属性 -->
			 <h2>{{computedFullName}}</h2>
			 <!-- 监听器 watch是监听数据变化-->
			 <h2>{{watchFullName}}</h2>
			 <h2>{{age}}</h2>
			 methods执行最多 computed和watch最敏感 而且computed是有缓存机制的
			 实例对象内的数据比如age methods computed watch不相关数据改变只执行methods 
			 实例对象内的数据比如firstName methods computed watch相关数据改变会全部执行
             实例对象外的数据 other改变不会触发methods computed watch
			 实例对象外的数据 other改变后再改变实例对象内的相关数据会触发methods computed watch
			 实例对象外的数据 other改变后再改变实例对象内的不相关数据会触发methods
		</div>
		<script src="https://cdn.jsdelivr.net/npm/vue@2.6.10/dist/vue.js"></script>
		<script>
		   var other = 'This is other';
			const app = new Vue({
				  ...
				  methods:{
					  getMethodsFullName(){
						  console.log("调用了方法getMethodsFullName");
						  return this.firstName+''+this.lastName+other
					  }, 
					  change(){
						  console.log("调用了方法change");
						  return this.watchFullName='111'
					  }
				  },
				  computed:{
					 computedFullName(){
						  console.log("调用了计算属性computedFullName");
					 	  return this.firstName+''+this.lastName+other
					 }
				  }, 
				  watch:{
